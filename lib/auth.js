...
import { findUserByEmail, findUserById, createUser, toPublicUser } from './authStore.js'
...
export async function getBearerUser(authHeader) {
...
export async function signUp({ email, password, name }) {
  ...
  const user = await createUser({ id: crypto.randomUUID(), email, name, passwordHash })

  const publicUser = toPublicUser(user)
  await trackEvent('signup', { userId: user.id, email: user.email, meta: { name: user.name } })
  await updateUserStats(user.id, { email: user.email, name: user.name, courseCount: 0, guideCount: 0, courses: [] })
...
export async function signIn({ email, password }) {
  ...
  await trackEvent('signin', { userId: user.id, email: user.email })
  await updateUserStats(user.id, { email: user.email, name: user.name })
...
export async function adminSignIn({ email, password }) {
  ...
  await trackEvent('admin_signin', { email: adminEmail })
